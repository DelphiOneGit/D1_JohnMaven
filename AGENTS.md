...
- Repo: https://github.com/DelphiOneGit/D1_JohnMaven
- John Maven is a CLI-based AI agent for deep financial research by Delphi One (www.delphi.one), built with TypeScript, Ink (React for CLI), and LangChain.
...
- Config: `.jmv/settings.json` (persisted model/provider selection)
...
- Version format: SemVer `MAJOR.MINOR.PATCH` (e.g., `1.0.1`). Tag prefix: `v`.
- Release script: `bash scripts/release.sh <major|minor|patch|x.y.z>`.
...
- Config stored in `.jmv/settings.json` (gitignored).
